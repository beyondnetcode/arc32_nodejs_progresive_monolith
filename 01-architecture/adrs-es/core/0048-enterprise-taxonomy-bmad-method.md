...
1. **The Blue-Map Layout:** La raíz del proyecto debe mantenerse plana y estrictamente dividida por capas de abstracción (`00-governance`, `01-architecture`, `02-domains`, `03-infrastructure`, `04-operations`, `05-knowledge`).
...
3. **Encapsulamiento DDD:** Todo el código fuente de las aplicaciones y librerías debe migrarse desde un enfoque técnico (ej. `apps/` y `libs/`) hacia un enfoque de Dominio (`02-domains/[nombre-del-dominio]`).
...
* **Escalabilidad Inmediata:** La clara separación en `02-domains` facilita la futura extracción de microservicios (Microservice Extraction Readiness).
...
* **Curva de Aprendizaje:** Los nuevos desarrolladores deben ser capacitados obligatoriamente sobre la política de taxonomía (ubicada en `00-governance/standards/repository-taxonomy.md`) antes de crear nuevas carpetas.
