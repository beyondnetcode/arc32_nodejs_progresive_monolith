...
1. **The Blue-Map Layout:** The project root must be kept flat and strictly divided by layers of abstraction (`00-governance`, `01-architecture`, `02-domains`, `03-infrastructure`, `04-operations`, `05-knowledge`).
...
3. **DDD Encapsulation:** All source code of applications and libraries must be migrated from a technical approach (e.g., `apps/` and `libs/`) to a Domain approach (`02-domains/[domain-name]`).
...
* **Immediate Scalability:** The clear separation in `02-domains` facilitates the future extraction of microservices (Microservice Extraction Readiness).
...
* **Learning Curve:** New developers must be obligatorily trained on the taxonomy policy (located in `00-governance/standards/repository-taxonomy.md`) before creating new folders.
