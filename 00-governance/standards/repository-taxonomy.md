...
## 🏗️ 1. Standard Directory Structure (The Blue-Map Layout)

```text
/ (Repository Root)
├── 📜 README.md                 # Executive Portal (Vision and initial navigation)
├── 📜 MASTER_INDEX.md           # Role-Based Routing
├── 📁 .bmad-core/               # ⚡ ENGINE: BMAD-Method implementation (Agents, Tooling)
├── 📁 .github/                  # 🤖 CI/CD: Workflows, Actions, Issue/PR Templates
├── 📁 .harness/                 # 🧠 AI CONTEXT: Base rules, Playbooks, Prompts
├── 📁 00-governance/            # ⚖️ LAWS: Policies, SDLC, and Standards
├── 📁 01-architecture/          # 🏗️ BLUEPRINTS: ADRs, Architecture, C4 Models
├── 📁 02-domains/               # 📦 BUSINESS: Source code encapsulated by domain (DDD)
├── 📁 03-infrastructure/        # 🛠️ FOUNDATION: Infrastructure as Code (IaC), DevOps
├── 📁 04-operations/            # 🚀 RUN: Operations Playbooks, Observability
└── 📁 05-knowledge/             # 🎓 LEARNING: Onboarding, POCs, Examples, Training
```
...
## 🗂️ 2. Taxonomy and Naming Conventions
...
2. **Docs-as-Code:** Forbidden to repeat standards; always link to `00-governance/`.
3. **Breadcrumbs:** Every deep Markdown document must contain a backlink to `MASTER_INDEX.md`.

## 🧩 4. Domain Separation (DDD)

The code in `02-domains/` is organized by **Business Capability**. Code inside `user-management` cannot directly import internal files from another domain. Inter-domain communication must be resolved via formal contracts (Interfaces, APIs, Events).
