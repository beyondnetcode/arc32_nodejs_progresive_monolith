...
*   âŒ `"Queries products."`
*   âœ… `"Retrieves the detailed catalog of active products. REQUIRED when the user asks for availability, prices, or stock levels. Do NOT use this for billing queries."`
...
*   âŒ `HTTP 500 Internal Server Error` (The agent gives up).
*   âœ… `{"error": "Invalid Format", "details": "Zip code must be 5 numeric digits. Found 'ABC4'. Please correct and retry."}` (The agent reasons, reformulates, and calls again successfully).
