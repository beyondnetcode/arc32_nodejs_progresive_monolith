...
## Consequences
*   âœ… **Dramatic reduction in initial hallucinations:** The agent knows exactly how to compile and which conventions to follow from the very first chat turn.
*   âœ… **AI Auto-Onboarding:** Facilitates seamless use of multiple agent CLI tools (Claude, Aider, Mentat).
*   âš ï¸ **Maintenance:** Humans must remember to update `AGENTS.md` if changing a critical test command, or risk disorienting the agent. It is recommended to add a rule within the file itself reminding the Agent to update it upon architectural changes.
