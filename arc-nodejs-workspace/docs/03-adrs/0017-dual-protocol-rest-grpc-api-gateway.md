# ðŸ“œ ADR-0027 â€” Dual-Protocol REST & gRPC API Structure with Kong Gateway
...
The To-Do Reference Template serves as the central identity, session, and authorization gatekeeper for multiple integrated SaaS platforms (such as SCM, ERP, and WMS). As an agnostically deployable security kernel, Reference Skeleton must satisfy two competing API integration requirements:

1. **Universal Client Compatibility:** Public client systems, browser portals, and mobile frontends must integrate seamlessly with Reference Skeleton using standard, universally accessible HTTP transport protocols with zero native SDK requirements.
2. **Sub-millisecond Internal Performance:** Internal service-to-service validation and permission graph compilation requests (e.g., BFF to Reference Core) must complete with minimal network overhead to satisfy our high-performance SLA of **p95 < 5ms**.
...
1. **Public REST (JSON) Interface:** All public client portals, administrative consoles, and the Reference Skeleton Hosted Login redirect endpoints will communicate via REST over HTTP/1.1 or HTTP/2, using standard JSON serialization and Swagger OpenAPI documentation.
