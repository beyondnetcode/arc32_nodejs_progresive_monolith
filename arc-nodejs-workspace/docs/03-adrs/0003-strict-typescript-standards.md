...
## Context
Code quality, security (OWASP Top 10), and type safety are critical pillars for Reference Skeleton. Permitting the `any` type leads to runtime exceptions, makes code reviews difficult, and degrades our compiler's safety nets. Additionally, we need a way to detect cognitive complexity, bugs, and security hotspots early, without incurring external license costs (such as SonarCloud's paid tier for private repositories).
